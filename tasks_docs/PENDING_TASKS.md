...
> **W 类遗留问题全部关闭（2026-08-14 体系化重构阶段 0–4）**：W1（in-process watchdog 先于外部
> supervisor 触发）阶段 0 根治、W2（T2 失焦）阶段 0 根治、W3（瞬时错误归因过宽）阶段 3 根治、
> W4（reviewer 散文判定）阶段 3 缓解、W5（verify RCE 面）阶段 2 根治、W6（交接 fail-open）达标。
> 完整记录见 `HANDOVER.md`"遗留问题清单"节与 `MAIN_TASKS.md`"重大决策记录"。
