"""Fleet driver: parallel, physically-isolated self-driving runs (concurrent fleet).
...
instance per workspace, DESIGN-worker-isolation.md), a fleet run drives several
full `Drive` stacks IN PARALLEL, each bound to its own workspace instance:
  ...
  * all tasks share ONE Reporter journal (single truth, thread-safe; attribution by
    workflow id), so the whole fleet reports to one board.
...
parallel threads. See docs/DESIGN-fleet.md.
"""
...
@dataclass
class FleetTask:
    """One task in a fleet run: its context and the workspace it runs in."""
...
class Fleet:
    ...
    def run(self, tasks: list[FleetTask], worker_count: int | None = None) -> dict:
        ...
        the executor is surfaced as an ERROR DriveResult (never hangs the fleet).
        """
        ...
        def _go(task: FleetTask) -> None:
            try:
                ...
                dr = DriveResult("error", detail=str(exc), supervisor="fleet_error")
            with lock:
