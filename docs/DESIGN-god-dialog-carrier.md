# 上帝对话框载体决策（DESIGN-god-dialog-carrier）

> 日期：2026-08-06 · 状态：调研已定案 · 依据：opencode 官方文档调研 + regime-driver 现状盘点。
> 决策：**opencode 作为人类对话载体 + 自研 GodDialogUnit 作为程序化面并行共存**，两者共用同一确定性后端与 CLI 契约。

## 1. 结论（可行性量化）

| 方案 | 可行性 | 说明 |
|---|---|---|
| opencode 作上帝对话框载体 | **高 ~85%** | 对话层/UI/权限/工具调用全由 opencode 提供；剩余主要是 CLI 契约升级 |
...
- **自定义 agent**：`opencode agent create` + 提示词 + 权限，可做 `god` primary agent。
...
2. **硬实时/确定性——做不到**：agent 走 LLM，非确定、延迟受模型控制；**安全兜底（宪法/不变量）必须留在确定性后端**。
...
```
              ...
              ┌───────────────▼────────────────┐   ┌──────────────────────────┐
              │ A路：opencode `god` agent      │   │ B路：自研 GodDialogUnit   │
