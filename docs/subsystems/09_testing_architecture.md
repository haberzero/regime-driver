...
1. **开发宿主永不充当被测执行器**：开发环境只产代码/测试；E2E 一律在容器 worker 上跑。
...
- **E2E 系统化**：`e2e_tests/test_e2e_worker.py`（REGIME_E2E 门控，真实 worker）。
- **对话面配置进容器**：`Dockerfile.dialog-control` + `docker/dialog-control-config/`
  （装 regime-driver + dialog-control.md + regime-dialog-control 插件，非 --pure，端口 4098）。
- **控制对话框 A 路 HTTP 驱动**：HTTP 建会话 → dialog-control 调用插件工具 → 真实控制
  （`regime_status` 返回真实 worker 健康，dialog-control 结构化报告）。
- **regime-driver 容器化**：可选（当前宿主驱动已够）。
...
1. **控制对话框会话宿主用独立 控制对话框容器**：新建 `opencode-dialog-control` 容器（`docker/Dockerfile.dialog-control`，端口
   ...
   方案：worker 纯净、dialog-control 独立，角色不混用。
