...
> 读完你应能理解：你的主要操作是**和控制对话框对话**，
> 对话背后是一整套确定性的角色与流程机制在替你执行。

## 为什么需要控制对话框

在 [首页](../index.md) 已说明：你想给 opencode 下达"元指令"（"每次写完代码先按质量原则检查"
...
你（人类）                    你只说话："帮我做一个先实现再审查的流程，跑起来"
    │
    ▼
控制对话框（对话操作面）   替你：设计流程、注册、校验、启动、监控、解释
    │
    ▼
...
- **它是独立的对话操作面**：不依赖特定 session 管理，所有操作通过一个对话框进行。
- **脑与嘴分离**：
...
**何时该打开对话框值守**（相对于只运行 `regime drive`）：
...
| design | reviewer | judge | 审查方案 |
| implement | developer | agent | 实现 |
| test | reviewer | judge | 按审查规则检查实现 |
| wrap | developer | agent | 收尾与清理 |
...
2. **design**（reviewer 判定）：体系把 developer 的方案与任务上下文一起发给 reviewer 会话，
   请它按审查规则审查方案，给出 `advance`（通过）或其它判定。
   过不了确定性门就**不前进**。
3. **implement**（developer 干活）：审查通过后，developer 在它的工作区实现。
4. **test**（reviewer 判定）：体系把实现结果发给 reviewer，它按代码审查规则检查。
5. **wrap**（developer 收尾）。
...
**为什么你的元指令能被执行**：因为流程是**编译后的结构**，独立于对话内容。每个节点
"谁来做、做什么、按什么规则"都是固定的；审查节点不过关就卡住不前进；监督器盯着整条运行。
所以"写完代码先检查"从一句可能被遗忘的话，变成了 `test` 节点里一定会执行的代码审查。

**审查判定是闭环**：judge 节点的判定先过一道确定性门，再采信：
...
这样你可以随时复盘：任务走了哪些节点、审查判定是什么、为什么这样走。
...
又自己续跑"——这属正常恢复机制。默认策略下停滞则直接终止。详见
...
- 控制对话框是 regime-driver 的**主要使用入口**。
- 你的指令被**制度驱动地规划**：结合需求、流程、能力，选定制度路径，不经执行对象自由发挥。
