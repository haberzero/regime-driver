...
> 读写 A 的文件，不会碰到 B 和 C。（并行任务实例端口从 4200 起分配，与默认单 worker 的 4097
> 不同——两者互不干扰。）
...
每个任务跑在各自工作区实例上，物理隔离。
并行任务共享一个 reporter journal，单一真源。
...
用 `regime report` 查看整支并行任务的宏观汇报。

```bash
conda run -n regime-driver regime report --journal /tmp/parallel.jsonl
```
...
控制对话框是主要使用入口（见《控制对话框》）。

## 深入指引

- 工作区隔离设计：`../subsystems/02_worker_isolation.md`
- 并行任务设计：`../subsystems/03_parallel.md`
