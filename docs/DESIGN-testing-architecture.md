# 测试架构与职责定位设计（DESIGN-testing-architecture）

> 目的：在动手前把**概念职责、运行位置、驱动方式**彻底理清，避免"哪个 opencode 是干嘛的"
> 这类混淆在未来造成大问题。尤其澄清：**开发 session ≠ 被测对象**；**E2E / 上帝对话框验证
> 走 HTTP 程序化驱动，而不是交互式 TUI**。
> 状态：分析定案（待实施）。
...
```
轴 A — 组件职责（这是"谁"）
  开发 session / worker 执行器 / 监督器 / 上帝对话框 / 报告总线
...
| **上帝对话框 A 路（god agent）** | **控制面**：经 regime CLI 控制/监控全系统 | 容器内 opencode（web/HTTP） | HTTP（非 TUI） | **E2E 目标** |
| **上帝对话框 B 路（`regime dialog`）** | 程序化控制面（REPL） | 宿主 | 交互 REPL / 被调用 | 单元测 |
| **Reporter / `regime report`** | 可观测性 | 宿主/容器 | CLI | 用 |

**职责唯一原则**：
- **只有 regime-driver 驱动 worker 做任务**；worker 绝不直接接人类/上帝对话框。
- **上帝对话框只控制**（经 regime CLI），绝不直接干任务。
...
### 3.2 上帝对话框 A 路（验证"控制/监控"完整链路）
```
[程序化 HTTP 驱动]  ──建 god-agent session──▶  容器内 opencode(挂载 god.md+regime-god.js)
   status → run → monitor(sessions/events) → session send → report
```
- **关键洞察**：opencode 的 `serve`/`web` 暴露与 worker 相同的 session/message HTTP API
  （= regime-driver 驱动 worker 那套）。所以 A 路 god agent **可经 HTTP 程序化建会话并交互**，
  **完全绕开交互式 TUI**（即绕开"opencode run 挂起"）。
- 需要的容器配置：把 `god.md` + `regime-god.js` 挂载/复制进一个 opencode 实例，
  且该实例具备 regime-driver conda env（god 工具 shell 出 `conda run -n regime-driver regime ...`）。
- 目的：验证 god agent 能经插件工具 + 权限门禁 + 手册真实控制/监控系统。

### 3.3 两条路由的分工（防混乱）
- **E2E** 只验证"执行能力"（worker 干得好不好）。
- **上帝对话框 E2E** 验证"控制能力"（god 控制得对不对、看得到看不到全局）。
- 二者用同一套 HTTP 机制，但**目标对象不同**：E2E 目标是 worker 的任务执行，god E2E 目标是 god 的控制行为。
...
2. **HTTP 是唯一验证驱动**：E2E / god 验证一律 HTTP 程序化；不依赖交互式 TUI。
3. **worker 只执行**：不装开发工具、不接人类，保持干净（防污染验证结果）。
4. **god 会话必须在"有 god 配置 + 有 regime-driver env"的实例里跑**，否则工具/门禁不完整。
5. **每个容器只承担一个角色**：worker=执行、autopilot=web 窗/（可选）god 宿主；不混用。
...
**职责不混乱的关键**：开发（本 session）≠ 执行（worker）≠ 控制（god）≠ 监督（supervisor）；
验证一律走 **HTTP 程序化驱动 + 容器里的干净实例**，绕开交互 TUI。E2E 验"执行"，god E2E 验"控制"，两路分工、同一机制。
