...
| 模型 | 优点 | 缺点 | 采用 |
|---|---|---|---|
| **线程 + 消息队列** | 兼容现有同步 `OpenCodeClient`/urllib；简单 | GIL 限 CPU 密集（此处以 I/O 为主，影响小）| ✅ 推荐渐进首版 |
| asyncio | 契合事件驱动/消息；大量 I/O 等待高效 | 需把 client 改异步（大改）| 备选（阶段3+）|
...
2. **并发模型**：线程+消息队列（保同步 client）vs asyncio（改异步 client）。→ 建议先线程。
