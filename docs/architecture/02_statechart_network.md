...
| 权限/信号逻辑 | 宪法单元（`watchdog` 角色）广播 STOP；CLI 写操作走 `--perm` 门禁 | 显式授权模型 |
| 宪法层可覆写 | 用户可注入自定义宪法状态机；根不变量（I1/I2/I3）仍由运行时强制 | 可注入自定义宪法状态机 |
...
1. **迁移路线**：渐进（现有同步模型上叠加总线，monitor 升级为宪法状态机）。
   阶段 1-4 已按此实施完成。
2. **并发模型**：线程 + 消息队列（保同步 client），而非 asyncio（改异步 client）——
   状态机数量少、I/O 密集，asyncio 无显著优势（见 §5.2）。
3. **根安全不变量**：可覆写具体宪法策略，但保留运行时强制的 3 条根不变量
   （I1/I2/I3 由 `Runtime.start` 强制，见 §6）。
4. **默认宪法**：保留"内置默认宪法状态机"（`app/constitution_unit.py`）作为新用户默认，
   自定义作为可选覆写。
