...
>
> **本项目处于积极开发中，当前版本 v0.1（无稳定 API/CLI 契约保证）。**
>
> - **稳定性**：支持 2h+ 长时间耐久运行（资源有界增长）与复杂工程任务套件；
...
> **Status / 状态**
>
> - 测试：全量 `python -m pytest` 绿（覆盖率以实跑为准）；真实 worker E2E 本地可用（`REGIME_E2E=1` 门控）。
> - 当前状态：内部核心功能（流程热编译/热加载、drive 一键栈、多实例隔离并行任务、控制对话框）在稳定演进；
>   对外供给就绪（模板进包 / scaffold / 单一真源 / 发布文档）；长期耐久（2h+ 真实运行）资源有界增长。
...
配置/流程规格）、**开发者指南**（架构/子系统/如何开发）。仓库内导航见 `docs/MAP.md`；
...
> **警告：本项目仍处于开发中（v0.1）。** 作者按现状（AS-IS）提供本软件，不对其适用性、
