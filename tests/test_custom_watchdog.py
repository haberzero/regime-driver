"""Stage-4b: user can inject their OWN watchdog/watchdog unit (overridable).

The whole point of making the watchdog a peer state machine is that a user
can supply their own watchdog logic instead of the built-in WatchdogUnit.
...
class UserWatchdog(ThreadedUnit):
    """A user-defined watchdog: stops a unit if any report's output grows
    beyond a threshold (totally custom policy, not the built-in one)."""

    def __init__(self, unit_id="my-watchdog", max_output=1000, bus=None):
