...
4. **初始化任务控制文档**：按 `task-control/README.md` 建立项目的四类关键文档（`MAIN_TASKS`、`PENDING_TASKS`、`WORKLOG`、`HANDOVER`）。
